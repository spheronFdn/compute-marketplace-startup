...
curl -Ls https://snap.nodex.one/artela-testnet/genesis.json > $DAEMON_HOME/config/genesis.json
curl -Ls https://snap.nodex.one/artela-testnet/addrbook.json > $DAEMON_HOME/config/addrbook.json

# Configure seeds
sed -i -e "s|^seeds *=.*|seeds = \"d1d43cc7c7aef715957289fd96a114ecaa7ba756@testnet-seeds.nodex.one:23410\"|" $DAEMON_HOME/config/config.toml

# Configure gas prices
sed -i -e "s|^minimum-gas-prices *=.*|minimum-gas-prices = \"0.025uart\"|" $DAEMON_HOME/config/app.toml
...
sed -i -e "s%^proxy_app = \"tcp://127.0.0.1:26658\"%proxy_app = \"tcp://127.0.0.1:23458\"%; \
          ...
          s%^prometheus_listen_addr = \":26660\"%prometheus_listen_addr = \":23466\"%" $DAEMON_HOME/config/config.toml
...
mv $DAEMON_HOME/data/priv_validator_state.json $DAEMON_HOME
rm -rf $HOME/.artelad/data
tar xf $HOME/${snap} -C $DAEMON_HOME
rm $HOME/${snap}
mv $HOME/priv_validator_state.json $DAEMON_HOME/data/
wget -qO $DAEMON_HOME/config/addrbook.json https://snapshots.theamsolutions.info/artela-addrbook.json
